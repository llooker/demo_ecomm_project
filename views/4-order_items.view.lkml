view: order_items {
  ...
  dimension: history_button {
    sql: ${TABLE}.id ;;
    html: <a href=
      "/explore/getting_started_with_lookml/order_items?fields=users.id,order_items.count,order_items.created_date,inventory_items.product_id,inventory_items.cost*&f[order_items.order_id]={{ value }}"
...
######## Timeframe and Duration Dimensions ########
  ...
  dimension_group: shipped {
    ...
    #this column is stored as a date object so we want to transform it to a timestamp
    sql: timestamp(${TABLE}.shipped_at) ;;
  }
  ...
  dimension_group: delivered {
    ...
    sql: timestamp(${TABLE}.delivered_at) ;;
  }
